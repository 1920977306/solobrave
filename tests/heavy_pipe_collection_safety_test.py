# -*- coding: utf-8 -*-
"""★ fix/pytest-heavy-pipe-collection: 验证 heavy_pipe_test.py 模块级 sys.exit / 重逻辑已收进 _init_ns.

老大原话 (2026-09-25):
  "修tests/heavy_pipe_test.py: 把L111附近模块级sys.exit(1)及模块级直接执行的heavy pipe
   流程收进if __name__=="__main__":守卫, 使import/pytest collection不再退出;
   原有独立运行(python3 tests/heavy_pipe_test.py)行为保持不变;
   确保该文件被pytest正常收集且其case通过, 不再需要--ignore.
   配相应测试, 全绿."

测试设计:
  1. test_heavy_pipe_test_module_import_no_exit
     验证: import tests.heavy_pipe_test 不再触发 SystemExit (之前会 sys.exit(1) 让 pytest 整个套退出).
...
def test_heavy_pipe_test_module_import_no_exit():
    ...
    sys.modules.pop('tests.heavy_pipe_test', None)
    sys.modules.pop('heavy_pipe_test', None)

    # 尝试 import; 修复前会触发 SystemExit, 修复后正常返回模块
    try:
        hpt = importlib.import_module('tests.heavy_pipe_test')
    except SystemExit as e:
        # 修复失败: import 触发 sys.exit(1)
        raise AssertionError(
            f'Import 触发了 SystemExit({e.code}) — 模块级 sys.exit(1) 没收到 _init_ns() 里, '
            f'pytest collection 会因这个退出. 请检查 tests/heavy_pipe_test.py 模块级直接执行.'
        )
        ...
        raise AssertionError(
            ...
            f'KS_PY 相对路径依赖 cwd. 请检查 tests/heavy_pipe_test.py 模块级文件读取.'
        )
    except Exception as e:
        raise AssertionError(
            f'Import 抛 {type(e).__name__}({e}) — 模块级不应该有这种执行代码. '
            f'请检查 tests/heavy_pipe_test.py 模块级代码.'
        )

    # 验证模块已加载
    assert 'tests.heavy_pipe_test' in sys.modules, 'import 后模块应在 sys.modules 中'
