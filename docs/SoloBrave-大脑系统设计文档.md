# SoloBrave 大脑系统设计文档（v3）

> 本文档描述 SoloBrave 大脑系统当前实现（v3）。v2→v3 的核心变更见 [3.3 关键逻辑改动](#33-关键逻辑改动)。部分旧章节仍在逐步更新中，以代码实现为准。
...
1. **记忆服务（v3）**：重建现有记忆系统，解决 20 个已知问题
   - 分池存储（核心记忆 + 日常记录）
   - **物理隔离**：活跃记忆与归档记忆分文件存储
   - 跨平台文件锁（`msvcrt` / `fcntl`）保证并发安全
   - 所有 API 强制认证
   - 过期归档而非删除，支持恢复
...
├── memories/               # 记忆系统 v3（按员工隔离）
│   ├── {empId}/
│   │   ├── memory.json     # 活跃记忆（core + daily）
│   │   └── archived.json   # 归档记忆（物理隔离）
│   └── consolidation_log.json  # 全局归纳日志
├── knowledge/              # 知识库文档
├── products/               # 商品库（index.json + {id}.json）
└── influencers/            # 达人库（index.json + {id}.json）
```
...
#### `memories/{emp_id}/memory.json`（活跃记忆）
```json
{
  "version": "3.0",
  "config": {"core_max": 100, "daily_max": 100, "daily_ttl_days": 30},
  "core": [
    {"id": "mem_xxx", "key": "core", "value": "用户喜欢极简风格", "time": 1700000000000, "source": "归纳", "priority": 5, "accessCount": 3}
  ],
  "daily": [
    {"id": "mem_yyy", "key": "auto", "value": "用户提到下周出差", "time": 1700000000000, "source": "AI提取", "context": "..."}
  ]
}
```
...
|---|---|---|---|
| **core** | 100 条 | 无 | 手动管理 |
| **daily** | 100 条 | 30 天 | 过期后**物理移入** `archived.json`（可恢复） |

> **v3 关键变更**：归档不再是 `daily` 池中的逻辑标记，而是物理移动到独立的 `archived.json` 文件，活跃池更干净、加载更快。

#### `memories/{emp_id}/archived.json`（归档记忆）
```json
{
  "version": "3.0",
  "archived": [
    {"id": "mem_zzz", "key": "auto", "value": "用户问天气", "time": 1699000000000, "archivedAt": 1700000000000, "archiveReason": "expired", "source": "AI提取"}
  ]
}
```
...
| `capacity` | 活跃记忆总数超限 |
| `manual` | 手动触发归档 |
| `consolidated` | 归纳合并后原始记忆归档 |

#### `memories/consolidation_log.json`（归纳日志）
记录每次归纳操作的来源记忆 ID、合并后新记忆 ID 和时间戳，便于审计。
...
为减少重复计算，匹配结果会缓存到商品/达人对象上，TTL 24 小时：

| 接口 | 说明 |
|---|---|
| `GET /api/products/:id/matches` | 读取商品的匹配达人缓存；为空或过期时实时计算并回写 `matched_influencers` |
| `GET /api/influencers/:id/matches` | 读取达人的匹配商品缓存；为空或过期时实时计算并回写 `matched_products` |
...
{
  "matched_influencers": [...],
  "matched_influencers_updated_at": 1700000000000
}

// 达人
{
  "matched_products": [...],
  "matched_products_updated_at": 1700000000000
}
```
...
| `search_products` | `query` (string) | `GET /api/products?q=...` | 按关键词搜索商品 |
| `search_influencers` | `query` (string) | `GET /api/influencers?q=...` | 按关键词搜索达人 |
| `get_product_detail` | `productId` (string) | `GET /api/products/:id` | 查询单个商品详情 |
| `get_influencer_detail` | `influencerId` (string) | `GET /api/influencers/:id` | 查询单个达人详情 |
...
| `get_product_matches` | `productId` (string) | `GET /api/products/:id/matches` | 读取商品的匹配达人缓存 |
| `get_influencer_matches` | `influencerId` (string) | `GET /api/influencers/:id/matches` | 读取达人的匹配商品缓存 |
...
1. 商品库 (/api/products)
   - 查询商品列表、按关键词搜索商品、查看单个商品详情
   - 录入新商品、编辑商品信息
   - 每个商品有：名称、SKU、分类、价格、库存、标签、状态

2. 达人库 (/api/influencers)
   - 查询达人列表、按关键词搜索达人、查看单个达人详情
   - 录入新达人、编辑达人信息
...
3. 匹配引擎 (/api/match)
   - 为指定商品智能匹配适合的推广达人
   - 为指定达人智能匹配适合推广的商品
   - 读取商品/达人的匹配缓存（24h TTL）
   - 匹配维度：分类、标签、价格区间、粉丝数、互动率

当你需要执行以上操作时，请在回复末尾使用以下格式（用户看不到这段标记）：
[TOOL_CALL]{"action":"list_products"}[/TOOL_CALL]
[TOOL_CALL]{"action":"search_products","params":{"query":"口红"}}[/TOOL_CALL]
[TOOL_CALL]{"action":"get_product_detail","params":{"productId":"prod_xxx"}}[/TOOL_CALL]
...
当前已实现 8 个读取/匹配/搜索类 action，未来可按相同协议扩展：

| 潜在 Action | 说明 |
|---|---|
| `create_product` | 录入新商品（需完整商品参数）—— 建议保留人工确认 |
| `update_product` | 更新商品信息 —— 建议保留人工确认 |
| `create_influencer` | 录入新达人 —— 建议保留人工确认 |
| `update_influencer` | 更新达人信息 —— 建议保留人工确认 |

**扩展原则：** 保持 `[TOOL_CALL]{action, params}[/TOOL_CALL]` 格式不变，只需在 `executeToolCall()` 中新增 case 分支，并在 `buildAgentToolsContext()` 中补充说明。写入类 action 建议增加二次确认机制，避免 AI 自动修改业务数据。
...
| **记忆系统** | `renderMemoryTab()`, `switchMemoryTab()` | v3 分池读写、物理归档、归纳日志 |
| **商品库** | `openProductLibrary()`, `renderProducts()`, `saveProduct()` | 录入/搜索/匹配入口，支持缓存 |
| **达人库** | `openInfluencerLibrary()`, `renderInfluencers()`, `saveInfluencer()` | 录入/搜索/匹配入口，支持缓存 |
| **匹配引擎** | `showMatchResultModal()`, `matchProductToInfluencer()`, `matchInfluencerToProduct()` | 弹窗展示匹配结果，双向缓存 |
| **知识库** | `openKnowledgeDocForm()`, `saveKnowledgeDoc()`, `deleteKnowledgeDoc()` | 记忆面板内 CRUD 管理 |
...
| GET | `/api/products/:id/matches` | 商品匹配达人缓存 | 是 |
...
| GET | `/api/influencers/:id` | 达人详情 | 是 |
| GET | `/api/influencers/:id/matches` | 达人匹配商品缓存 | 是 |
...
记忆系统是 SoloBrave 最核心的差异化能力之一。v3 在 v2 基础上引入**物理隔离归档**、**跨平台文件锁**、**归纳日志**等增强机制。具体实现参见 `memory_service_v3.py`。
...
| 过期处理 | 物理删除 | **物理移入** `archived.json`，可恢复 |
| 并发安全 | 无锁 | 跨平台文件锁（`msvcrt.locking` / `fcntl.flock`） |
| 认证缺失 | 直接读取 | 所有 API 强制 `_authenticate()` |
| 版本兼容 | 扁平数组 | 自动迁移到 v3 分池格式 |
| 注入失控 | 全量注入 | 分层截断（core 全部 + daily 5 + archive 2 + knowledge 3） |
| 归纳审计 | 无 | `consolidation_log.json` 记录每次合并操作 |
...
│  · 上限 100 条，超出拒绝写入              │
│  · 无 TTL，永不自动过期                  │
│  · 文件：memories/{empId}/memory.json    │
├─────────────────────────────────────────┤
│  L2 日常记录池 (daily)                   │  ← 自动提取，30 天 TTL
│  · 聊天中 AI 自动提取的短期记忆           │
│  · 上限 100 条，超出拒绝写入              │
│  · 30 天未访问 → 物理移入 archived.json  │
│  · 文件：memories/{empId}/memory.json    │
├─────────────────────────────────────────┤
│  L3 归档层 (archive)                     │  ← 物理隔离，非逻辑标记
│  · 过期/手动/容量/归纳归档的记忆          │
│  · 文件：memories/{empId}/archived.json  │
│  · 可恢复、可删除、仅作为 L3 补充注入     │
└─────────────────────────────────────────┘
```
...
**活跃记忆文件 (`memories/{emp_id}/memory.json`)：**
...
**归档记忆文件 (`memories/{emp_id}/archived.json`)：**
...
| `source` | string | 来源标记 |
| `time` | int | 毫秒时间戳 |
| `priority` | int | core 记忆优先级 1-10 |
| `tags` | array | 标签数组 |
| `accessCount` | int | 访问计数，用于注入排序 |
| `context` | string | daily 记忆上下文摘要 |
| `expiresAt` | int | daily 记忆过期时间 |
| `archivedAt` | int | 归档时间（仅 archive） |
| `archiveReason` | string | 归档原因（仅 archive） |

> **注意**：v3 采用物理隔离。`memory.json` 只保存活跃记忆，`archived.json` 独立存储归档记忆。后端 `GET /api/memory/:empId` 会自动加载两者并合并返回。
...
```python
def _cleanup_and_archive_expired(emp_id):
    now = int(time.time() * 1000)
    ttl_ms = cfg['daily_ttl_days'] * 24 * 3600 * 1000
    
    data = load_memory(emp_id)       # 读活跃记忆
    archive_data = load_archive(emp_id)  # 读归档文件
    ...
    if expired:
        ...
        archive_data['archived'].extend(expired)
        save_memory(emp_id, data)
        save_archive(emp_id, archive_data)
```

**关键设计：**
- **物理归档**：从 `memory.json` 的 `daily` 池中**移除**，写入 `archived.json`
- **活跃池更干净**：加载活跃记忆时无需遍历已归档数据
- **可恢复**：通过 `POST /api/memory/:empId/:memId/restore` 从 `archived.json` 移回 `daily`
...
if active_total > 200 and daily:
    oldest = min(daily, key=lambda m: m.get('time', 0))
    oldest['archivedAt'] = now
    oldest['archiveReason'] = 'capacity'
    archive_data['archived'].append(oldest)
    data['daily'].remove(oldest)
```

### 4.6 四层注入策略

记忆注入发生在 AI 对话时，由 `memory_service_v3.inject_memories()` 统一执行。

**注入优先级（从高到低）：**

```
1. 核心记忆 (L1)
   · 全部注入（上限 50 条，实际由 core_max 控制）
   · 按 priority 降序、accessCount 升序排序
   · 每条截断至 inject_value_max (500) 字符
   
2. 日常记录 (L2)
   · 按时间倒序取最新 inject_daily_max (5) 条
   · 仅取活跃 daily（已归档的不在活跃池）
   · 每条截断至 500 字符
   
3. 归档补充 (L3)
   · 当需要补充时从 archived.json 取最近归档
   · 最多 inject_archive_max (2) 条，前缀标记 [归档]
...
- 跨平台：Windows 用 `msvcrt.locking`，Unix 用 `fcntl.flock`
- 多进程安全：文件锁可被操作系统识别，不仅限于同进程线程
...
| | 记忆归档 | 聊天记录归档 |
|---|---|---|
| 触发条件 | 30 天 TTL / 容量 / 手动 / 归纳 | 超过 500 条 |
| 存储位置 | `memories/{emp}/archived.json` | `memories/{emp}/archived.json` summaries |
| 数据形式 | 单条记忆对象 | 对话摘要文本 |
