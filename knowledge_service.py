...
def kb_entries_reindex_pending():
    """★ refactor/kb-vectorization-error-handling: 批量重建未向量化的知识条目
    (status='pending' / 'embedding_failed' / chunk_count=0):
    重新分段 + 向量化; 成功时 ok/error/embedding_failed 条目恢复 ok, pending 保持 pending 不自动过审;
    失败时 vectorize 错误已被 helper 标 'embedding_failed' (不覆盖), save_chunks 错误标 'error';
    无 API key 保持原状态。返回 {'total', 'ok', 'noKey', 'failed', 'errors'} 统计。
    """
    conn = _db_conn()
    try:
        rows = conn.execute(
