...
def compute_field_stats(runs, ground_truth):
    ...
    for key in all_keys:
        ...
        # ★ fix/mini-test-code-repair-20260925 P1-3: 复杂类型 (dict/list) 不可哈希,
        #   用 json.dumps 转字符串后再判 set (sort_keys 保证顺序无关)
        def _hashable(v):
            if isinstance(v, (dict, list, tuple)):
                try:
                    ...
                    return repr(v)
            return v
        non_null_hashable = [_hashable(v) for v in non_null]
        consistent = len(set(non_null_hashable)) <= 1
        # 系统性错 (多次都错同一个错值, >= 3 次)
        if non_null and truth is not None:
            truth_hash = _hashable(truth)
            wrong_values_hashable = [h for h, v in zip(non_null_hashable, non_null) if v != truth]
            all_wrong_unique = list(set(wrong_values_hashable))
            systematic = len(all_wrong_unique) == 1 and len(wrong_values_hashable) >= 3
        else:
            all_wrong_unique = list(set(non_null_hashable))
            systematic = False
...
def main():
    ...
    parser.add_argument('--input', required=True, help='原图路径 (jpg/png, base64 编码后调 server._call_kimi_vision)')
    ...
    # ★ fix/mini-test-code-repair-20260925 P1-1: 删除 OCR dump 加载分支
    #   之前 main() 把 --input 当 JSON dump 加载, 但 vision_rescan_single() 吃图片 (base64). 矛盾.
    #   修后 --input 直接传给 vision_rescan_single() 当图片路径, 不再 json.load.
